...
reshape_exp_data <- function(db, metadata, mother_tbl) {
  ...
  DESIGN_str <-
    merge_tbls(list(DESIGN = DESIGN_tbl),
               ...
               type = "child-parent",
               drop_keys = FALSE)[[1]] %>%
    dplyr::select(any_of(std_str_names), Faktor1_Stufe_ID, Faktor2_Stufe_ID)  #!!
  ...
  MNGT_fmt <- lapply(names(MNGT_ipt), function(df_name){
    ...
    df <- df %>%
      ...
      dplyr::select(-c(ID_trt_1, ID_trt_2, ID_fix, is_trt, Faktor1_Stufe_ID, Faktor2_Stufe_ID)) %>%
  ...
  TREATMENTS_matrix <- TREATMENTS_matrix %>%
    dplyr::select(-c(Plot_id, "Treatment_id")) %>%
    distinct()
